...
$(document).ready(function () {
	...
	$('img#refreshTask').live('click', function () {
		loadTask();
	});
	...
	function startEndTask(url, formData) {
		...
		$.ajax({
			...
			success: function success(data) {
				...
				if ($('#startEndTask').attr('name') == 'start') $('#startEndTask').val('Done');else {
					if ($('#startEndTask').hasClass('mytask')) $('#startEndTask').parent().parent().parent().remove();else $('#startEndTask').remove();
				}
			},
	...
	$(".fui").click(function () {
		$("div.lol").animate({ height: 300 }, "slow");
	});
	$("#newshead_right").click(function () {
		$(".conto").slideToggle();
	});

	$.getJSON(serviceURL + 'ressources/test', function (data) {
    ...
    */
	...
	var removeLastPart = function removeLastPart(url) {
	...
	function sendata(action, user_id) {
		...
		if (action == "in") var meth = "insert";else var meth = "update";
	...
	$('input..datepickerDob').live('focus', function () {
		...
		$('.datepickerDob').datepicker({ dateFormat: 'yy-mm-dd',
			changeMonth: true,
			changeYear: true,
			yearRange: "-40:-16"
